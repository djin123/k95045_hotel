layui.use(['jquery','layer','table','form','laydate'], function() {
    ...
    function loadOrders(){
        table.render({   //每一次查询重新渲染table容器
            ...
            ,cols: [[ //表头  field: 'id'表示从实体对象的属性中取到数据放入容器里
                ...
                ,{field: 'roomPic', title: '客人名称', width:120, align:'center',sort: true,templet:'<div>{{d.inRoomInfo.customerName}}</div>'}
                ,{field: 'roomTypeName', title: '身份证号', width:220,align:'center',templet:'<div>{{d.inRoomInfo.idcard}}</div>'}
                ,{field: 'roomPrice', title: 'vip', width: 80,align:'center', sort: true,templet:'#isVipTpl'}
                ,{field: 'customerName', title: '手机号', width: 180,align:'center',templet:'<div>{{d.inRoomInfo.phone}}</div>'}
    ...
    //刷新数据表格
    function flush() {   //layui在此处如果将where参数传回服务器，则后面再修改参数值就无法再变
                         //flow流加载容器也存在这种情况
        table.reload('demo', {
